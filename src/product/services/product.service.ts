...
export class ProductService {
    ...
    async create(productRequest: ProductRequestDto): Promise<ProductResponseDto> {
        const isProductExist = await this.productModel.findOne({ name: productRequest.name });
        if (isProductExist) {
    ...
    async findAll(page: number = 0, limit: number = 10): Promise<ListAllProductsDto> {
        const products = await this.productModel.find().skip(page * limit).limit(limit).lean();
        if (!products.length) {
